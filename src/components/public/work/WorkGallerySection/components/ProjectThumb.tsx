...
export function ProjectThumb({
  ...
  const pid =
    p.id ??
    p.generalInfos?.slug ??
    p.generalInfos?.title ??
    p.generalInfos?.heroUrl ??
    "";
  ...
  return (
    <Link
      ...
      aria-label={p.generalInfos.title}
    >
      <div className={cn("relative w-full", ratioClass)}>
        {!imgError && p.generalInfos.heroUrl ? (
          <Image
            // Keyed to identity to avoid stale images when list changes
            key={pid}
            src={p.generalInfos.heroUrl}
            alt={p.generalInfos.title}
            fill
            sizes={computedSizes}
        ...
        <div className="absolute top-2 left-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          {p.generalInfos.tags.map((t, ix) => (
            <span
              key={ix}
              className="rounded-full border-2 border-black/10 px-2 py-1 text-xs bg-white/90 backdrop-blur"
            >
              {t}
            </span>
          ))}
